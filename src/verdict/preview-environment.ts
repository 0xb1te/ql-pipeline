 ...
 */
export const PREVIEW_CONTRACT_NODE =
  'ql-docs `workflow/rules/stage-8-deployment/` — *Preview environment contract* ' +
...
export type PreviewEnvironmentVerdict =
  ...
  | { readonly kind: 'valid' }
  | { readonly kind: 'invalid'; readonly violations: readonly string[] };
...
export function requiresPreviewEnvironment(appDirs: readonly string[], areaPaths: AreaPathsConfig): boolean {
...
export function previewEnvironmentViolations(snapshot: PreviewEnvironmentSnapshot): readonly string[] {
    ...
    if (!isRecord(parsed)) {
      ...
      if (!isRecord(services) || Object.keys(services).length === 0) {
        ...
        if (!isRecord(services[PREVIEW_EDGE_SERVICE])) {
          violations.push(
            `\`${PREVIEW_COMPOSE_FILE}\` has no service named \`${PREVIEW_EDGE_SERVICE}\`. The contract routes ` +
        ...
        if (publishing.length > 0) {
          violations.push(
            `\`${PREVIEW_COMPOSE_FILE}\` publishes host ports on ${publishing.join(', ')}. A preview is reached ` +
  ...
  if (!snapshot.envExamplePresent) {
    violations.push(
      `\`${PREVIEW_ENV_EXAMPLE}\` is missing. It must list every variable the stack needs, with no real ` +
...
export function assessPreviewEnvironment(
  snapshot: PreviewEnvironmentSnapshot,
  areaPaths: AreaPathsConfig,
): PreviewEnvironmentVerdict {
...
export function previewEnvironmentFinding(verdict: PreviewEnvironmentVerdict): Finding | null {
...
export function readPreviewEnvironmentSnapshot(root: string, reader: SnapshotReader = DISK): PreviewEnvironmentSnapshot {
