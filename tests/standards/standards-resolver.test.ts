...
function reader(files: Record<string, string>): StandardsReader {
  return {
    ...
    read: vi.fn((path: string) => files[path] ?? ''),
  };
}
...
describe('resolveStandards', () => {
  it('loads the configured document for a matched area', () => {
    const files = reader({ [at('workflow/stage-5-frontend/checklist.md')]: '# Frontend checklist' });

    const { standards } = resolveStandards(['frontend'], config(), ROOT, files);
  ...
  it('concatenates several documents for one area, labelling each source', () => {
    const files = reader({
      [at('workflow/stage-4-backend/backend/checklist.md')]: 'BACKEND RULES',
      [at('workflow/stage-4-backend/sql/checklist.md')]: 'SQL RULES',
    });

    const { standards } = resolveStandards(['backend'], config(), ROOT, files);
  ...
  it('returns one entry per area for a PR spanning several', () => {
    const files = reader({
    ...
    const { standards } = resolveStandards(['frontend', 'backend'], config(), ROOT, files);

    expect(standardsIds(standards)).toEqual(['frontend.standards', 'backend.standards']);
  });

  it('reports a configured document that is not checked out, rather than silently skipping it', () => {
    const files = reader({});

    const { standards, missing } = resolveStandards(['frontend'], config(), ROOT, files);

    expect(standards).toEqual([]);
    expect(missing).toEqual(['workflow/stage-5-frontend/checklist.md']);
  });

  it('still applies the documents it does find when only some are missing', () => {
    const files = reader({ [at('workflow/stage-4-backend/backend/checklist.md')]: 'BE' });

    const { standards, missing } = resolveStandards(['backend'], config(), ROOT, files);

    expect(standards).toHaveLength(1);
    expect(missing).toEqual(['workflow/stage-4-backend/sql/checklist.md']);
  });

  it('loads nothing at all when standards are disabled', () => {
    const files = reader({ [at('workflow/stage-5-frontend/checklist.md')]: 'FE' });

    expect(resolveStandards(['frontend'], config({ enabled: false }), ROOT, files)).toEqual({
      standards: [],
      missing: [],
    });
  ...
  it('skips an area that has no documents configured', () => {
    const files = reader({});

    const { standards, missing } = resolveStandards(['docs'], config(), ROOT, files);

    expect(standards).toEqual([]);
    expect(missing).toEqual([]);
  });

  it('marks a document truncated when it exceeds the per-area budget', () => {
    const huge = `## 01 — First\n${'x'.repeat(500)}\n## 02 — Second\n${'y'.repeat(500)}`;
    const files = reader({ [at('workflow/stage-5-frontend/checklist.md')]: huge });

    const { standards } = resolveStandards(['frontend'], config({ maxCharsPerArea: 400 }), ROOT, files);
...
describe('formatStandardsForPrompt', () => {
  ...
  it('tells the reviewer exactly how to cite each document', () => {
