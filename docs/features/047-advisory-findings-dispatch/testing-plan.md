...
| TASK047-17 | A governed PR whose review raises only advisory findings dispatches an agent | Threads carry a pickup reply and a fix attempt runs | STILL PENDING 2026-09-22, and the original plan for it was wrong. ql-desktop #103 merged rather than being reopened, and its findings could never have closed this row anyway. OBSERVED LIVE: four governed pull requests since this merged - ql-desktop #105, #106, #107 and #109 - each reported advisory… | PENDING | Gate 3. Cannot be forced: it needs a review to volunteer an auto-fixable `should`. As of 2026-09-22 GitHub Actions is refusing to start jobs on this account for billing, so no governance run of any kind can be observed until that clears. |  |
