...
function headerIndexes(header: readonly string[]): Map<string, number> {
  const found = new Map<string, number>();
  header.forEach((cell, index) => {
    const name = cell.trim();
    if (name !== '' && !found.has(name)) found.set(name, index);
  });
  return found;
}
...
export function parseTestPlan(bytes: Buffer): readonly TestCase[] {
  ...
  if (rows === undefined) {
    ...
    throw new TestPlanError(
      `testing-plan.xlsx has no "${MCP_SHEET_NAME}" sheet (it has ${names || 'no sheets'}). ` +
  ...
  if (absent.length > 0) {
    throw new TestPlanError(
      `the "${MCP_SHEET_NAME}" sheet is missing required column(s): ${absent.join(', ')}. ` +
        `Row 1 must be the header row. Found: ${header.map((h) => `"${h}"`).join(', ')}`,
    );
  }

  const at = (row: readonly string[], name: string): string => (row[index.get(name) ?? -1] ?? '').trim();

  const cases: TestCase[] = [];
  const seen = new Set<string>();
  rows.slice(1).forEach((row, offset) => {
    const rowNumber = offset + 2;
    if (row.every((cell) => cell.trim() === '')) return; // spreadsheet padding, not a case
    ...
    const blank = (['Case-id', 'Call', 'Expected result', 'Severity'] as const).filter((name) => at(row, name) === '');
    if (blank.length > 0) {
    ...
    if (!(SEVERITIES as readonly string[]).includes(severityCell)) {
      throw new TestPlanError(
        `row ${String(rowNumber)}: Severity must be one of ${SEVERITIES.join(', ')} (got "${at(row, 'Severity')}")`,
      );
    }
    ...
    if (rawInput !== '') {
      try {
        ...
        throw new TestPlanError(
          `row ${String(rowNumber)}: Input is not valid JSON. One cell, one JSON value — got ${rawInput.slice(0, 120)}`,
        );
      }
    }
    ...
    cases.push({
      ...
      input,
      expected,
      severity: severityCell as CaseSeverity,
      row: rowNumber,
    });
  });
