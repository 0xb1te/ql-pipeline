...
- **Gate note:** delivered from a written brief that pre-scoped the work. Gate 1 is recorded as closed on that brief; Gate 2 is the pull request. Gate 3 is the run on this pull request itself, whose govern log should carry `preview environment: not required — no apps/*frontend* or apps/*backend* directory`.
