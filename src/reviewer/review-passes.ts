...
export function dedupeFindings(passes: readonly (readonly Finding[])[]): Finding[] {
  ...
  const anchoredByAnEarlierPass = new Set<string>();
  const merged: Finding[] = [];

  for (const pass of passes) {
    const anchorsThisPass = new Set<string>();
    const keysThisPass = new Set<string>();
