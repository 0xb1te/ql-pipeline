...
export function determineRoute(input: RouteInput, config: PipelineConfig): RouteResult {
  ...
  if (parsed.length > 0) {
    return buildDecision(
      parsed.map((commit) => commit.type),
      parsed.map((commit) => commit.area),
      config,
    );
  }
  ...
  return {
    ok: false,
    reason:
      `no commit on this PR matches the required header format \`<type>(<area>): <description>\`, ` +
      `and the PR title ("${input.prTitle}") doesn't either`,
  };
}

function buildDecision(types: readonly CommitType[], areas: readonly Area[], config: PipelineConfig): RouteResult {
  ...
  const gates: AreaGate[] = uniqueAreas.map((area) => {
...
function dedupeInCanonicalOrder<T>(values: readonly T[], canonicalOrder: readonly T[]): T[] {
