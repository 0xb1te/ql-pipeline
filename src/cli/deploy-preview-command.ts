...
import {
  ...
  previewUpArgs,
} from '../deploy/preview-stack.js';
import { defaultArgvExecutor, type ArgvExecutor } from '../shared/exec.js';
...
import { createPipelineContext, type PipelineContext } from './bootstrap.js';
...
async function probeOverMcp(endpoint: string): Promise<boolean> {
  try {
...
function messageOf(cause: unknown): string {
  if (cause instanceof Error) {
    const withStreams = cause as Error & { stderr?: unknown; stdout?: unknown };
...
export async function deployPreview(ctx: Context, deps: DeployPreviewDeps): Promise<DeployPreviewOutcome> {
  ...
  if (proxyHome === undefined || proxyHome.trim() === '') {
    return {
      kind: 'failed',
      reason:
        `${PROXY_HOME_VAR} is not set. It names the preview host's built checkout of ql-proxy and is a ` +
  ...
  if (listing !== null) {
    try {
      const containers = await deps.exec(
        'docker',
        ['compose', '-p', listing.project, 'ps', '-q', config.preview.mcp.service],
        { cwd: consumerRoot },
      );
      ...
      if (containerId === '') {
        ...
        const inspected = await deps.exec(
          'docker',
  ...
  if (endpoint !== null) {
    ...
    for (;;) {
      ...
      if (ready || deps.now() >= deadline) break;
  ...
  const expiresAt =
    listing?.minutesRemaining === null || listing?.minutesRemaining === undefined || !listing.expires
      ? null
...
export async function runDeployPreview(): Promise<void> {
