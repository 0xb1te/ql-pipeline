...
The pipeline routes, labels, gates, AI-reviews, and auto-merges clean PRs today (Phase 3). A PR with fixable findings fails its check rather than looping — the auto-fix agent doesn't exist yet (Phase 4). See the task list above for what's actually implemented.
...
src/main.ts              Entrypoint the reusable workflow runs: route, label, gate, review, decide, merge
