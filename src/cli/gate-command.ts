...
import { serializeGateReport, type GateReport } from '../shared/gate-report.js';
import type { AreaGate, GateOutcome, RouteDecision } from '../shared/types.js';
...
export async function runGateStage(stage: GateStage, reportPath: string): Promise<void> {
  ...
  const outcomes: GateOutcome[] = await runGates(gates, { cwd: consumerRoot });
  writeGateReport(reportPath, { stage, outcomes });
  ...
  if (failed.length > 0) {
    core.setFailed(
      `${failed.length} ${stage} gate(s) failed: ${failed.map((outcome) => outcome.area).join(', ')}`,
    );
  }
}
