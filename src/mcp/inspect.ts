...
import { createGithubClient, type GithubClient } from '../shared/github-client.js';
import type * as GovernCommand from '../cli/govern-command.js';
import type { Finding, GateOutcome, PipelineConfig, Severity } from '../shared/types.js';
...
const ROOT_PROPERTY = {
  type: 'string',
  description:
    'Absolute path to the repo to inspect. Defaults to this MCP server process\'s own working directory, ' +
...
const REPORTS_DIR_PROPERTY = {
  type: 'string',
  description:
    'Directory holding the gate jobs\' JSON reports, relative to `root` unless absolute. Defaults to ' +
...
const FINDINGS_PROPERTY = {
  type: 'array',
  description:
    'AI review findings to weigh alongside the gates. Omit to ask what the gates alone decide. Each entry ' +
...
export const INSPECT_TOOLS: readonly ToolDescriptor[] = [
  {
    name: 'ql_pipeline_route',
    description:
      'Read-only. Answers "where would this pull request route, and why": the areas its conventional-commit ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        configPath: {
          type: 'string',
          description:
            'Explicit pipeline config path. Defaults to $PIPELINE_CONFIG_PATH, then .github/pipeline.config.yml, ' +
  ...
  },
  {
    ...
    description:
      'Read-only. Reads back what the build and test gate jobs left behind for a run — per area and stage: the ' +
  ...
  },
  {
    ...
    description:
      'Read-only. The question `govern` exists to answer, without govern\'s side effects: given the gate reports ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        commitMessages: {
          type: 'array',
          description:
            "The PR's commit messages. Used only to count prior fix attempts, the same way the pipeline counts " +
  ...
  },
  {
    ...
    description:
      'Read-only. Lists the open pull requests this pipeline has parked on a person: `needs-human` (it stopped ' +
...
async function resolveReader(deps: InspectDeps): Promise<ReadGateReports> {
...
function readStringArray(params: Record<string, unknown>, key: string): readonly string[] {
  const value = params[key];
 ...
 */
const CONFIG_CANDIDATES = ['.github/pipeline.config.yml', 'pipeline.config.yml'] as const;
...
function resolveConfigPath(
  ...
  return {
    ok: false,
    reason:
      `no pipeline config found under "${root}" (looked for ${CONFIG_CANDIDATES.join(' and ')}). ` +
...
function parseFindings(value: unknown): { ok: true; findings: Finding[] } | { ok: false; reason: string } {
  if (value === undefined) {
...
function describeGateOutcomes(outcomes: readonly GateOutcome[]): Record<string, unknown>[] {
...
export async function runInspectTool(name: string, args: unknown, deps: InspectDeps = {}): Promise<ToolResult | null> {
...
): { ok: true; config: PipelineConfig; path: string } | { ok: false; result: ToolResult } {
...
function resolveReportsDir(params: Record<string, unknown>, root: string): string {
  const named = readString(params, 'reportsDir') ?? 'gate-reports';
  return isAbsolute(named) ? named : join(root, named);
}

function gateReportsTool(
  params: Record<string, unknown>,
  root: string,
  readReports: ReadGateReports,
): ToolResult {
...
function verdictTool(
  ...
  const attemptsSoFar = countFixAttempts(
    readStringArray(params, 'commitMessages'),
    readStringArray(params, 'commentBodies'),
  );
  ...
  return ok({
    ...
    note:
      'read-only: this is what `govern` would decide from the same inputs. Nothing was merged, ' +
      'commented, labelled or pushed.',
  });
}

async function humanQueueTool(
  params: Record<string, unknown>,
  env: Readonly<Record<string, string | undefined>>,
  createClient: (token: string) => Pick<GithubClient, 'listPullRequestsByLabel'>,
): Promise<ToolResult> {
  ...
  try {
    const byLabel = await Promise.all(
      labels.map(async (label) => ({ label, pulls: await client.listPullRequestsByLabel({ owner, repo }, label) })),
    );
