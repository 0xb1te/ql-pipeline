...
function readCentralDirectory(buf: Buffer): Map<string, ZipEntry> {
  ...
  if (eocd === -1) throw new XlsxError('not a zip archive: no end-of-central-directory record');
...
function columnIndexOf(ref: string): number {
  ...
  if (letters === undefined) throw new XlsxError(`unreadable cell reference "${ref}"`);
  let index = 0;
      ...
      if (type === 'inlineStr') {
        ...
        if (raw !== undefined) {
          ...
          if (type === 's') {
            ...
            if (resolved === undefined) throw new XlsxError(`shared string ${decoded} is out of range`);
            value = resolved;
          } else if (type === 'b') {
...
// @signal readWorkbook
export function readWorkbook(bytes: Buffer): Map<string, string[][]> {
  const entries = readCentralDirectory(bytes);

  const workbookXml = entries.get('xl/workbook.xml');
  if (workbookXml === undefined) throw new XlsxError('not an xlsx workbook: xl/workbook.xml is missing');
  const relsEntry = entries.get('xl/_rels/workbook.xml.rels');
  if (relsEntry === undefined) throw new XlsxError('not an xlsx workbook: xl/_rels/workbook.xml.rels is missing');
